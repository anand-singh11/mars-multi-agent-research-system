# mars/agents.py
"""
Agents module containing all agent nodes (supervisor, researcher, writer, critiquer).
"""
...
if not os.environ.get("GROQ_API_KEY"):
    os.environ["GROQ_API_KEY"] = getpass.getpass("Groq API key (GROQ_API_KEY):\n")
...
# llama-3.3-70b-versatile: 30 RPM free tier (vs Gemini's 5 RPM)
# Ideal for multi-agent workflows with many sequential LLM calls.
llm = ChatGroq(
...
def create_supervisor_chain():
    ...
    def supervisor_invoke(state):
        ...
        try:
            ...
            if text.startswith("```"):
                lines = text.split("\n")
                text = "\n".join([line_text for line_text in lines if not line_text.strip().startswith("```")])
            text = text.strip()
